...
@pytest.mark.django_db
def test_get_form_passes_serializer_to_context(client):
    url = reverse("legal_entities:legal-entity-create")
    response = client.get(url, HTTP_ACCEPT="text/html")
    assert response.status_code == status.HTTP_200_OK
...
@pytest.mark.django_db
def test_form_post_legal_person_redirects_to_success(client):
    url = reverse("legal_entities:legal-entity-create")
    data = {
...
@pytest.mark.django_db
def test_form_post_natural_person_redirects_to_success(client):
    url = reverse("legal_entities:legal-entity-create")
    data = {
...
@pytest.mark.django_db
def test_form_post_invalid_passes_serializer_with_errors(client):
    url = reverse("legal_entities:legal-entity-create")
    data = {"entity_type": "legal_person", "country_code": "SE"}  # missing legal_name
    response = client.post(url, data, HTTP_ACCEPT="text/html")
    assert response.status_code == status.HTTP_400_BAD_REQUEST
