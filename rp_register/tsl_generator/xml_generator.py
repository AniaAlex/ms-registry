...
def generate_tsl_xml_etsi_format(scheme) -> str:
    ...
    # TSL Sequence Number
    lines.append(f"        <TSLSequenceNumber>{scheme.sequence_number}</TSLSequenceNumber>")
    ...
    for name in scheme.operator_names.all():
        lines.append(f'            <Name xml:lang="{name.language}">{_escape_xml(name.value)}</Name>')
    lines.append("        </SchemeOperatorName>")
    ...
    if first_provider and first_provider.street_address:
        lines.append(f"                    <StreetAddress>{_escape_xml(first_provider.street_address)}</StreetAddress>")
        if first_provider.locality:
            lines.append(f"                    <Locality>{_escape_xml(first_provider.locality)}</Locality>")
        if first_provider.postal_code:
            lines.append(f"                    <PostalCode>{_escape_xml(first_provider.postal_code)}</PostalCode>")
        lines.append(f"                    <CountryName>{scheme.territory}</CountryName>")
    else:
    ...
    for uri in scheme.information_uris.all():
        lines.append(f'                <URI xml:lang="{uri.language}">{_escape_xml(uri.uri)}</URI>')
    lines.append("            </ElectronicAddress>")
    lines.append("        </SchemeOperatorAddress>")
    ...
    # Status Determination Approach
    lines.append(f"        <StatusDeterminationApproach>{_escape_xml(scheme.status_determination_approach)}</StatusDeterminationApproach>")
    
    # Scheme Type Community Rules
    if scheme.community_rules.exists():
        lines.append("        <SchemeTypeCommunityRules>")
        for rule in scheme.community_rules.all():
            lines.append(f'            <URI xml:lang="{rule.language}">{_escape_xml(rule.uri)}</URI>')
        lines.append("        </SchemeTypeCommunityRules>")
    ...
    if scheme.legal_notices.exists():
        ...
        for notice in scheme.legal_notices.all():
            lines.append(f'            <TSLLegalNotice xml:lang="{notice.language}">{_escape_xml(notice.notice)}</TSLLegalNotice>')
        lines.append("        </PolicyOrLegalNotice>")
    
    # Historical Information Period
    lines.append(f"        <HistoricalInformationPeriod>{scheme.historical_information_period}</HistoricalInformationPeriod>")
        ...
        for pointer in scheme.pointers.all():
            ...
            if pointer.certificates.exists():
                ...
                for cert in pointer.certificates.all():
                    ...
                    lines.append(f"                            <X509Certificate>{cert.certificate_data}</X509Certificate>")
                    lines.append("                        </DigitalId>")
                    lines.append("                    </ServiceDigitalIdentity>")
                lines.append("                </ServiceDigitalIdentities>")
            
            lines.append(f"                <TSLLocation>{_escape_xml(pointer.tsl_location)}</TSLLocation>")
            
            # Additional Information
            lines.append("                <AdditionalInformation>")
            lines.append("                    <OtherInformation>")
            lines.append(f"                        <TSLType>{pointer.tsl_type}</TSLType>")
            lines.append("                    </OtherInformation>")
            lines.append("                    <OtherInformation>")
            lines.append(f"                        <SchemeTerritory>{pointer.scheme_territory}</SchemeTerritory>")
            lines.append("                    </OtherInformation>")
            lines.append("                    <OtherInformation>")
            lines.append(f"                        <ns3:MimeType>{pointer.mime_type}</ns3:MimeType>")
            lines.append("                    </OtherInformation>")
            if pointer.operator_names.exists():
                lines.append("                    <OtherInformation>")
                lines.append("                        <SchemeOperatorName>")
    ...
    for provider in scheme.providers.filter(is_active=True):
        ...
        if provider.street_address:
            lines.append("                    <PostalAddresses>")
            lines.append('                        <PostalAddress xml:lang="en">')
            lines.append(f"                            <StreetAddress>{_escape_xml(provider.street_address)}</StreetAddress>")
            if provider.locality:
                lines.append(f"                            <Locality>{_escape_xml(provider.locality)}</Locality>")
            if provider.state_or_province:
                lines.append(f"                            <StateOrProvince>{_escape_xml(provider.state_or_province)}</StateOrProvince>")
            if provider.postal_code:
                lines.append(f"                            <PostalCode>{_escape_xml(provider.postal_code)}</PostalCode>")
            lines.append(f"                            <CountryName>{_escape_xml(provider.country_name)}</CountryName>")
            lines.append("                        </PostalAddress>")
            lines.append("                    </PostalAddresses>")
        ...
        if provider.electronic_addresses.exists():
            lines.append("                    <ElectronicAddress>")
            for addr in provider.electronic_addresses.all():
                lines.append(f'                        <URI xml:lang="{addr.language}">{_escape_xml(addr.uri)}</URI>')
            lines.append("                    </ElectronicAddress>")
        ...
        for service in provider.services.filter(is_active=True):
            ...
            # Service Type Identifier
            lines.append(f"                        <ServiceTypeIdentifier>{service.service_type}</ServiceTypeIdentifier>")
            ...
            if service.certificates.exists():
                ...
                for cert in service.certificates.all():
                    ...
                    lines.append("                            <DigitalId>")
                    lines.append(f"                                <X509Certificate>{cert.get_base64_der()}</X509Certificate>")
                    lines.append("                            </DigitalId>")
                    # Output X509SubjectName if available
                    if cert.x509_subject_name:
                        lines.append("                            <DigitalId>")
                        lines.append(f"                                <X509SubjectName>{_escape_xml(cert.x509_subject_name)}</X509SubjectName>")
                        lines.append("                            </DigitalId>")
                    # Output X509SKI if available
                    if cert.x509_ski:
                        lines.append("                            <DigitalId>")
                        lines.append(f"                                <X509SKI>{cert.x509_ski}</X509SKI>")
                        lines.append("                            </DigitalId>")
                lines.append("                        </ServiceDigitalIdentity>")
            
            # Service Status
            lines.append(f"                        <ServiceStatus>{service.status}</ServiceStatus>")
            
            # Status Starting Time
            status_time = service.status_starting_time.strftime("%Y-%m-%dT%H:%M:%SZ")
            lines.append(f"                        <StatusStartingTime>{status_time}</StatusStartingTime>")
            ...
            has_extensions = (
                ...
                hasattr(service, 'additional_service_info') and service.additional_service_info.exists()
            )
            
            if has_extensions or service.supply_points.exists() or service.definition_uris.exists():
                lines.append("                        <ServiceInformationExtensions>")
                
                # Qualifications extension
                if hasattr(service, 'qualifications') and service.qualifications.exists():
                    lines.append('                            <Extension Critical="true">')
                    lines.append("                                <ns5:Qualifications>")
                    for qual in service.qualifications.all():
                        lines.append("                                    <ns5:QualificationElement>")
                        lines.append("                                        <ns5:Qualifiers>")
                        lines.append(f'                                            <ns5:Qualifier uri="{_escape_xml(qual.qualifier_uri)}"/>')
                        lines.append("                                        </ns5:Qualifiers>")
                        lines.append(f'                                        <ns5:CriteriaList assert="{qual.criteria_assert}">')
                        # Add key usage bits if specified
                        if qual.key_usage:
                            lines.append("                                            <ns5:KeyUsage>")
                            for ku_name, ku_value in [
                                ...
                                lines.append(f'                                                <ns5:KeyUsageBit name="{ku_name}">{str(ku_value).lower()}</ns5:KeyUsageBit>')
                            lines.append("                                            </ns5:KeyUsage>")
                        lines.append("                                        </ns5:CriteriaList>")
                        lines.append("                                    </ns5:QualificationElement>")
                    lines.append("                                </ns5:Qualifications>")
                    lines.append("                            </Extension>")
                
                # Additional Service Information extension
                if hasattr(service, 'additional_service_info') and service.additional_service_info.exists():
                    for info in service.additional_service_info.all():
                        critical = "true" if info.critical else "false"
                        lines.append(f'                            <Extension Critical="{critical}">')
                        lines.append("                                <AdditionalServiceInformation>")
                        lines.append(f'                                    <URI xml:lang="{info.language}">{_escape_xml(info.uri)}</URI>')
                        lines.append("                                </AdditionalServiceInformation>")
                        lines.append("                            </Extension>")
            ...
            if service.history.exists():
                ...
                for hist in service.history.all().order_by('-status_starting_time'):
                    ...
                    lines.append(f"                            <ServiceTypeIdentifier>{hist.service_type}</ServiceTypeIdentifier>")
                    lines.append("                            <ServiceName>")
                    for name in hist.names.all():
                        lines.append(f'                                <Name xml:lang="{name.language}">{_escape_xml(name.value)}</Name>')
                    lines.append("                            </ServiceName>")
                    ...
                    if hasattr(hist, 'digital_ids') and hist.digital_ids.exists():
                        ...
                        for digital_id in hist.digital_ids.all():
                            ...
                            if digital_id.x509_subject_name:
                                lines.append(f"                                    <X509SubjectName>{_escape_xml(digital_id.x509_subject_name)}</X509SubjectName>")
                            lines.append("                                </DigitalId>")
                            if digital_id.x509_ski:
                                lines.append("                                <DigitalId>")
                                lines.append(f"                                    <X509SKI>{digital_id.x509_ski}</X509SKI>")
                                lines.append("                                </DigitalId>")
                        lines.append("                            </ServiceDigitalIdentity>")
                    
                    lines.append(f"                            <ServiceStatus>{hist.status}</ServiceStatus>")
                    hist_time = hist.status_starting_time.strftime("%Y-%m-%dT%H:%M:%SZ")
                    lines.append(f"                            <StatusStartingTime>{hist_time}</StatusStartingTime>")
                    
                    # History Service Information Extensions
                    has_hist_extensions = (
                        (hasattr(hist, 'qualifications') and hist.qualifications.exists()) or
                        (hasattr(hist, 'additional_info') and hist.additional_info.exists())
                    )
                    ...
                    if has_hist_extensions:
                        ...
                        if hasattr(hist, 'qualifications') and hist.qualifications.exists():
                            lines.append('                                <Extension Critical="true">')
                            lines.append("                                    <ns5:Qualifications>")
                        ...
                        if hasattr(hist, 'additional_info') and hist.additional_info.exists():
